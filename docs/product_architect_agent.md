...
* **The Constraint:** The app runs on a free **Render** instance (subject to 15-minute cold starts) and connects to a free **Neon.tech** Serverless Postgres instance. Outbound connection limits to third-party sports metrics providers (like Sportmonks v3) are strictly limited.
...
* **The Logic:**
  ...
  * If empty, execute an isolated downstream sync to Sportmonks, serialize the response mapping placeholder brackets gracefully (e.g., "Winner Group A"), populate Postgres, and stream the data back to the client.
...
* Apply explicit structural indexes on query fields via entity decorators or migrations: `fixtures(match_date_time)` and `followed_teams(team_name)` to prevent full-table scans during scheduled notification checks.
...
* Mock database queries and external Axios calls when validating data mappings. Ensure that every edge case (such as Sportmonks returning unfinalized knockout states like "TBD" or empty arrays) is gracefully verified through unit tests before proposing implementation steps.
