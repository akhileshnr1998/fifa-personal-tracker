...
const WORLD_CUP_2026_MATCHES: SeedMatch[] = [
...
const SEED_ID_END = SEED_ID_START + WORLD_CUP_2026_MATCHES.length - 1;
...
function getMatchNumber(match: SeedMatch, groupMatchNumber: number): number {
  ...
  return groupMatchNumber;
}

export class SeedWorldCup2026Fixtures1749523300000 implements MigrationInterface {
  name = 'SeedWorldCup2026Fixtures1749523300000';

  public async up(queryRunner: QueryRunner): Promise<void> {
    let groupMatchNumber = 1;
    const values = WORLD_CUP_2026_MATCHES.map((match, index) => {
      const id = SEED_ID_START + index;
      const matchNumber = getMatchNumber(match, groupMatchNumber);
      if (match.num === undefined && match.round.startsWith('Matchday')) {
        groupMatchNumber += 1;
      }
      const stageId = getStageId(match.round);
      const matchDateTime = parseMatchDateTime(match.date, match.time);

      return `(${id}, ${matchNumber}, '${matchDateTime}', ${stageId}, '${escapeSql(match.team1)}', '${escapeSql(match.team2)}', '${escapeSql(match.ground)}')`;
    }).join(',\n');

    await queryRunner.query(`
      INSERT INTO fixtures (id, match_number, match_date_time, stage_id, home_team, away_team, venue)
      VALUES ${values}
    `);
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`
      ...
      WHERE id BETWEEN ${SEED_ID_START} AND ${SEED_ID_END}
    `);
  }
}
